...
LAUNCH_SINGLE_BAR() {
	...
	polybar -q apps -c    	  "$DIR"/config &
	polybar -q monitoring -c  "$DIR"/config &
	polybar -q bspwmInfo -c   "$DIR"/config &
	polybar -q info -c   "$DIR"/config &
	polybar -q SystemTray -c  "$DIR"/config &
...
LAUNCH_MULTI_BAR() {
	...
	for MON in $(polybar --list-monitors | cut -d":" -f1);
		...
		do
			...
			MONITOR=$MON polybar -q apps -c    	   "$DIR"/config &
			MONITOR=$MON polybar -q monitoring -c  "$DIR"/config &
			MONITOR=$MON polybar -q bspwmInfo -c   "$DIR"/config &
			MONITOR=$MON polybar -q info -c        "$DIR"/config &
			MONITOR=$MON polybar -q SystemTray -c  "$DIR"/config &
